...
def main(page: flet.Page):
    ...
    def on_dialog_result(e:flet.FilePickerResultEvent):
        if e.files != None:
            ...
            file_path.value = ''
            file_path.update()
            #could get rid of file_name box if find a new location for error message?
    ...
    ORF = flet.ElevatedButton(text="Generate ORF File", icon=flet.icons.FILE_DOWNLOAD_ROUNDED, on_click=generate_ORF_file)
    ...
    tm_dropdown = flet.Dropdown(bgcolor = flet.colors.WHITE, label="Select Tm method:", hint_text="Should Tm be included in the header?",
    ...
    if length>1 and sys.argv[1].endswith(tuple(ext)):
        ...
        #w and GC text fields are placeholders for values, they are also inclued in the page to add spacing between icons ven though they do not display any data
        page.add(file_picker, file_name, file_path, f, t, string_tb, b, w, re_enzyme_search, re, GC, ORF, gd, c)
